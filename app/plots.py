...
class TimelinePlot(ABC): # Hardcoded to watering for now
    ...
    def get_fig(self): 
        ...
        sorted_items = [d for d in sorted(self.items, key=lambda i: i['watering_frequency'])]
        for y_item, item in enumerate(sorted_items, start=1): 
            ax.scatter(x=item['dates_watered'], y=[y_item for i in item['dates_watered']], alpha=.6, s=50, color=mapper.to_rgba(item["watering_frequency"]))
            y_labels.append(f"{item['name']}\n{1/item['watering_frequency']: .0f} days avg.")
